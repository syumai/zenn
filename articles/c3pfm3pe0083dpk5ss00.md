...
このコードでは、defer 文により遅延実行された関数の中で、名前付き結果パラメータの値を操作しています。

この時の挙動を知るためのポイントは下記の3つです。
...
雑な図で恐縮ですが、関数宣言と照らし合わせると下記のような構成になっています（厳密なものではありません）
...
* **パラメータ (parameters)**
  - 関数自身が受け付けるパラメータのこと。上記図中での `(a int)` に対応する
* **結果パラメータ (result parameters)**
  - 関数 / メソッドを呼び出した結果として呼び出し元に返されるパラメータのこと。上記図中での `(b int)` に対応する
* **名前付きパラメータ / 名前付き結果パラメータ (named [result] parameters)**
  - 説明したパラメータ / 結果パラメータに対して、名前を付与したもの。
    - パラメータ / 結果パラメータは名前無しで `(a int) => (int)`、 `(b int) => int` と書くことが出来る[^2]。

本記事中での呼称も上記の通りとなります。

[^2]: 関数宣言でパラメータの名前を省略すると、そのパラメータを関数内で使うことが出来なくなるので基本的に省略されない。結果パラメータの名前は省略されることが多い
...
ざっくり訳すと、 *関数が返るとき、関数の戻りパラメータが呼び出し元に値で渡される。* といった事が書かれています。
...
この時、 `return 200, nil` は、名前付き結果パラメータの変数 `i` に対して `200` を、 `err` に対して `e`を設定します。
関数は、実行終了時点で結果パラメータの変数に格納されている値を返すので、 `200` , `e` の2つを返します。
...
もしかしたら、このような実装になっているGo言語処理系も存在しているかも知れません。
:::
...
このような場合、`defer func(){ ... }()` の呼び出しは、return 文が結果パラメータの変数 `i` に値を設定した後に行われます。
そのため、出力結果は `100 * 2` で `200` となります。

さらに、もう一つ特筆すべき性質があります。
ここまで説明した中に、下記のようなものがありました。

* 結果パラメータは、関数内では通常のローカル変数と同じように扱うことができる
* 関数は、関数の実行終了時点で結果パラメータに設定されている値を返す

これらと、defer 文の *return 文が呼び出された後、関数が終了する前に実行される* 性質を組み合わせると、 **defer 文は、結果パラメータに設定された値を変更して、関数の返す値を操作することが出来る** と言えます。
...
ここまでで、今回の問題を解くために知っている必要のある性質は全て説明しました！
